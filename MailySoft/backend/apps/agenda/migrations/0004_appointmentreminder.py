# Generated by Django 5.2.14 on 2026-06-03 14:13

import django.db.models.deletion
import uuid
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ("agenda", "0003_fix_overlap_constraints"),
        ("tenancy", "0003_alter_tenantmembership_unique_together_and_more"),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name="AppointmentReminder",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4, editable=False, primary_key=True, serialize=False
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "deleted_at",
                    models.DateTimeField(
                        blank=True,
                        db_index=True,
                        help_text="NULL = activo. Rellenar para borrado lógico.",
                        null=True,
                    ),
                ),
                (
                    "channel",
                    models.CharField(
                        choices=[("whatsapp", "WhatsApp"), ("sms", "SMS"), ("email", "Email")],
                        default="whatsapp",
                        help_text="Canal de envío del recordatorio.",
                        max_length=20,
                    ),
                ),
                (
                    "scheduled_at",
                    models.DateTimeField(
                        db_index=True, help_text="Momento UTC en que debe enviarse el recordatorio."
                    ),
                ),
                (
                    "sent_at",
                    models.DateTimeField(
                        blank=True,
                        help_text="Momento UTC en que se envió efectivamente. Null si aún no se envió.",
                        null=True,
                    ),
                ),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("pending", "Pendiente"),
                            ("sent", "Enviado"),
                            ("failed", "Fallido"),
                            ("skipped", "Omitido"),
                            ("cancelled", "Cancelado"),
                        ],
                        db_index=True,
                        default="pending",
                        help_text="Estado del recordatorio.",
                        max_length=20,
                    ),
                ),
                (
                    "message_preview",
                    models.TextField(
                        blank=True,
                        default="",
                        help_text="Primeros ~500 caracteres del mensaje enviado (para trazabilidad).",
                    ),
                ),
                (
                    "error_detail",
                    models.TextField(
                        blank=True, default="", help_text="Detalle del error si el envío falló."
                    ),
                ),
                (
                    "external_message_id",
                    models.CharField(
                        blank=True,
                        default="",
                        help_text="ID del mensaje asignado por el proveedor externo (Meta, Twilio, etc.). Útil para reconciliación y webhooks de entrega.",
                        max_length=200,
                    ),
                ),
                (
                    "appointment",
                    models.ForeignKey(
                        help_text="Cita a la que corresponde este recordatorio.",
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="reminders",
                        to="agenda.appointment",
                    ),
                ),
                (
                    "created_by",
                    models.ForeignKey(
                        blank=True,
                        help_text="Usuario que creó el registro. Null en imports/seeds o si el usuario fue borrado.",
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        related_name="+",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
                (
                    "tenant",
                    models.ForeignKey(
                        help_text="Clínica a la que pertenece este registro.",
                        on_delete=django.db.models.deletion.PROTECT,
                        related_name="+",
                        to="tenancy.tenant",
                    ),
                ),
            ],
            options={
                "db_table": "agenda_appointment_reminders",
                "ordering": ["scheduled_at"],
                "indexes": [
                    models.Index(
                        fields=["scheduled_at", "status"], name="reminder_scheduled_status_idx"
                    ),
                    models.Index(fields=["tenant", "appointment"], name="reminder_tenant_appt_idx"),
                ],
            },
        ),
    ]
